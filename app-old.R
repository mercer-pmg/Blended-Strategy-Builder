...
sleeve_table2 <- eventReactive(input$submit, {
     ...
     x <- strategy_current()
     y <- readr::read_csv("Model Agg - Asset Class Exposures.csv", show_col_types = FALSE) |>
          dplyr::mutate(
               asset_class = asset_class |> stringr::str_replace_all("\xa0", " ")
          )
     
     x <- dplyr::left_join(x,y, by = dplyr::join_by(sleeve == model_agg)) |>
          dplyr::select(asset_class, weight)
...
p <-  reactive({ggplot2::ggplot(data = sleeve_table2(),
                                ggplot2::aes(x = 3, y = weight, fill = asset_class)) +
          ggplot2::geom_col() +
...
output$donut <- renderPlot({
     if(weightSum() == 100){
          ggplot2::ggplot(data = sleeve_table2(),
                          ggplot2::aes(x = 3, y = weight, fill = asset_class)) +
               ggplot2::geom_col() +
               ggplot2::coord_polar(theta = "y") +
               ggplot2::xlim(c(0.2, 3.5)) +
               ...
               ggplot2::theme(
                    ...
                    axis.ticks       = ggplot2::element_blank(),
                    axis.text        = ggplot2::element_blank(),
                    legend.text      = ggplot2::element_text(size = 18, family = "IBM Plex Sans"),
                    legend.position  = "right",
                    legend.title     = ggplot2::element_blank()) 
     } else {
...
output$downloadReport <- downloadHandler(
     ...
     content = function(file) {
          ...
          tempImages <- file.path(tempdir(), "gdp-contributions.png")
          file.copy("Report.Rmd", tempReport, overwrite = TRUE)
          
          # Set up parameters to pass to Rmd document
          params <- list(allocation = strategy_current() |> dplyr::select(sleeve, weight),
                         plot       = p())
